...
public class UserRepositoryImpl implements UserRepository{
    ...
    public void loadData() {
        factory.getReactiveConnection().serverCommands().flushAll().thenMany(
                        Flux.just("U$ER 01", "u$eR 02", "us3r o3")
                                .map(name -> User.builder().id(String.valueOf(new Random().nextInt(0,10))).name(name).access(new Random().nextBoolean()).build())
                                .flatMap(user -> operations.opsForValue().set(user.getId(), user))).subscribe();
    ...
    @Override
    public Mono<User> findByAccess(boolean access) {
        return null;
    }

    @Override
    public Mono<Boolean> hasAccessById(String id) {
        return null;
    }

    @Override
    public Mono<Boolean> hasAccessByName(String name) {
        return null;
    }
